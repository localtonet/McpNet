...
namespace McpNet.Gateway.Aggregation
{
    public class ToolAggregator
    {
        ...
        /// <summary>BM25 full-text search index over all enabled tools.</summary>
        public ToolSearchIndex SearchIndex { get; } = new ToolSearchIndex();

        /// <summary>Cumulative metrics for the <c>mcpnet__retrieve_tools</c> meta-tool.</summary>
        public ToolSearchMetrics SearchMetrics { get; } = new ToolSearchMetrics();
        ...
        public async Task RefreshAsync(CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                // Rebuild BM25 index so retrieve_tools reflects the latest tool set.
                SearchIndex.Rebuild(GetEnabledTools());
            }
            finally
            {
                _isRefreshing = false;
